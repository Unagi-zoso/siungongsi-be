...
import org.bob.siungongsi.repository.TermsRepository;
...
public class AuthService {
  ...
  public AuthService(AuthRepository authRepository, TermsRepository termsRepository) {
  ...
  public List<TermsResponse> getTerms() {
    List<TermsEntity> terms = termsRepository.findAllByOrderByIdAsc();

    if (terms.isEmpty()) {
      throw new CustomException(
              ApiResponseCode.AUTH_TERMS_NOT_FOUND,
              ApiResponseCode.AUTH_TERMS_NOT_FOUND.getMessage());
    }

    return terms.stream()
            .map(term -> {
              String titleWithRequiredFlag = term.getRequiredFlag() == 1 ?
                      term.getTitle() + " (필수)" : term.getTitle() + " (선택)";
